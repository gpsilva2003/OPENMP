...
int main(int argc, char *argv[]) { /* off_calcpi.c  */
double passo = 1.0 / (double) num_passos;
double inicio, fim, soma = 0.0; 

   inicio = omp_get_wtime();
   // Offloading com OpenMP para o acelerador (GPU)
   #pragma omp target teams distribute parallel for reduction(+: soma) map(tofrom: soma) device(1)
   for (long int i = 0; i < num_passos; i++) {
      double x = (i + 0.5) * passo;
      soma += 4.0 / (1.0 + x * x);
   }
   double pi = soma * passo; // calcula o valor final de Pi
   fim = omp_get_wtime();
